...
public final class SmoothAssets {
    ...
    private static final Identifier LOGO_ID = Identifier.of(RaidMineStaffMod.MOD_ID, "runtime/rm_logo_hq");

    private static final EnumMap<UiIcon, TextureAsset> ICONS = new EnumMap<>(UiIcon.class);
    ...
    public static boolean ensureLogoInitialized() {
        ...
        synchronized (SmoothAssets.class) {
            ...
            try (InputStream stream = SmoothAssets.class.getResourceAsStream(
                ...
                if (stream == null) throw new IllegalStateException("RM Tools logo resource is missing");
                NativeImage image = NativeImage.read(stream);
                logoWidth = Math.max(1, image.getWidth());
                logoHeight = Math.max(1, image.getHeight());
                MinecraftClient.getInstance().getTextureManager().registerTexture(
                        LOGO_ID, new LinearTexture(() -> "RM Tools HQ logo", image));
                logoInitialized = true;
                return true;
            } catch (Throwable throwable) {
                logoFailed = true;
                RaidMineStaffMod.LOGGER.error("Could not create RM Tools logo texture", throwable);
                return false;
            }
        }
    }
    ...
    private static TextureAsset ensureIcon(UiIcon icon) {
        TextureAsset existing = ICONS.get(icon);
        if (existing != null) return existing;
        synchronized (ICONS) {
            existing = ICONS.get(icon);
            if (existing != null) return existing;
            String file = icon.name().toLowerCase(Locale.ROOT);
            String resource = "/assets/raidmine_staff/textures/icons/ui_hq/" + file + ".png";
            try (InputStream stream = SmoothAssets.class.getResourceAsStream(resource)) {
                ...
                NativeImage image = NativeImage.read(stream);
                Identifier id = Identifier.of(RaidMineStaffMod.MOD_ID, "runtime/icon_hq_" + file);
                int width = image.getWidth();
                int height = image.getHeight();
                MinecraftClient.getInstance().getTextureManager().registerTexture(
                        id, new LinearTexture(() -> "RM Tools HQ icon " + file, image));
                TextureAsset created = new TextureAsset(id, width, height);
                ICONS.put(icon, created);
                return created;
            } catch (Throwable throwable) {
                RaidMineStaffMod.LOGGER.error("Could not load HQ icon {}", icon, throwable);
                return null;
            }
        }
    }
    ...
    public static int drawBrandText(DrawContext context, String text, float x, float y,
        ...
        return font("hemico").draw(context, text, x, y, size, color, false);
    }

    public static int brandTextWidth(String text, float size) {
        return font("hemico").width(text, size, false);
    }
    ...
    private static String selectedFontKey() {
        ...
        return switch (configured.trim().toUpperCase(Locale.ROOT)) {
            case "ETUDE_NOIRE", "ETUDE NOIRE" -> "etude_noire";
            case "SEENONIM" -> "seenonim";
            default -> "hemico";
        };
    }
